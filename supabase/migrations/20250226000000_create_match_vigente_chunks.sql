-- Asegura que la función RPC match_vigente_chunks exista (RAG).
-- Requiere: tablas instrument_chunks, instrument_versions, instruments con columna embedding vector(1536).
...
GRANT EXECUTE ON FUNCTION public.match_vigente_chunks(vector(1536), int) TO anon;
GRANT EXECUTE ON FUNCTION public.match_vigente_chunks(vector(1536), int) TO authenticated;
GRANT EXECUTE ON FUNCTION public.match_vigente_chunks(vector(1536), int) TO service_role;
